"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TariffsController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const tariffs_service_1 = require("./tariffs.service");
let TariffsController = class TariffsController {
    constructor(tariffsService) {
        this.tariffsService = tariffsService;
    }
    addTariff(request) {
        return this.tariffsService.addTariff(request);
    }
    addTariffOppotunity(request) {
        return this.tariffsService.addTariffOppotunity(request);
    }
    getTariffsInfo(request) {
        return this.tariffsService.getTariffsInfo(request);
    }
};
__decorate([
    (0, common_1.Post)('/add-tariff'),
    __param(0, (0, common_1.Req)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], TariffsController.prototype, "addTariff", null);
__decorate([
    (0, common_1.Post)('/add-tariffOppotunity'),
    __param(0, (0, common_1.Req)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], TariffsController.prototype, "addTariffOppotunity", null);
__decorate([
    (0, common_1.Get)('/getTariffsInfo'),
    __param(0, (0, common_1.Req)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], TariffsController.prototype, "getTariffsInfo", null);
TariffsController = __decorate([
    (0, common_1.Controller)('tariffs'),
    (0, swagger_1.ApiTags)('Тарифы'),
    __metadata("design:paramtypes", [tariffs_service_1.TariffsService])
], TariffsController);
exports.TariffsController = TariffsController;
//# sourceMappingURL=tariffs.controller.js.map